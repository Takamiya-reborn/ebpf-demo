...
**重要**：`client`（[client/main.py](../../client/main.py#L1)）只执行 `config/tool_configs.json` 白名单中、位于 `my_tools/bin/` 的非符号链接可执行文件，以 `sudo -n` 无交互方式运行，期望工具输出为可解析的 `key: value` 或单行 JSON 字典。

## 目录摘要

- `Makefile`：按分类（`cpu/disk/file/memo/network`）构建 BPF 程序与用户态加载器，输出到 `bin/`。构建流程使用 `clang -target bpf` 编译 `.bpf.c`，并用 `bpftool` 生成 skeleton；用户态程序静态链接 `libbpf`（`staticobjs/libbpf.a`），产物不依赖系统 libbpf 动态库。
...
注意：多数工具会以 `sleep(1)` 或 `sleep(2)` 的周期轮询 BPF maps 并输出增量/快照；有些工具在输出后会清理 map（以实现差分统计）。用户态加载器统一处理 `SIGINT`/`SIGTERM` 并优雅退出——client 在达到时长上限或浏览器断开连接时会向工具进程组发送 SIGTERM（2 秒后仍未退出则 SIGKILL），新工具应保持这一约定。
...
- disk\_\*：带单位后缀的 key，例如 `disk_write_vfs_total_us: 12.345`、`disk_write_vfs_count: 3`。
...
client 侧的解析约束（`parse_output_metrics`）：key 长度超过 128 或数值非有限（NaN/Inf）的条目会被忽略；单次运行输出超过 2 MiB 或 20,000 行会被截断（截断处插入 `[output truncated]`）。工具应避免高频输出海量日志行。
...
- `irq_stat` (`cpu/irq_stat.c`)
  ...
  - attach 失败时输出错误信息并以非零退出码退出。

- `disk_write_delay` / `disk_read_delay` (`disk/*.c`)
  - 通过 kprobe 附加 `vfs_write` / `vfs_read` 以及 block 层相关函数，maps 聚合累计耗时与计数。
  - 任一 kprobe 附着失败时输出错误并退出，不再静默运行。
  - 用户态每秒读取累加值并与上次快照差分，输出如 `disk_write_vfs_total_us`（浮点）与 `disk_write_vfs_count`（整数）；差分带无符号下溢保护，map 读取失败或计数回退时按 0 增量处理。
...
- `mmap_stat` / `page_fault_stat` / `page_swap_stat` (`memo/*.c`)
  ...
  - `page_fault_stat`：以 `comm_pid: count` 格式输出并删除条目，响应 `SIGINT`/`SIGTERM` 优雅退出。
...
- `socket_stat` / `tcp_connect` (`network/*.c`)
  ...
  - `net_udp_stat` 的 kretprobe 只统计返回值 > 0 的 `udp_recvmsg` 成功接收，失败的调用不计入次数与字节数。
