...
int main(int argc, char **argv)
{
    ...
    fprintf(stderr, "Monitoring direct page reclaim by PID, press Ctrl+C to stop.\n");
    ...
    while (1) {
        ...
        while (bpf_map__get_next_key(skel->maps.counter, &pid, &next_pid, sizeof(pid)) == 0) {
            ...
            if (bpf_map__lookup_elem(skel->maps.counter, &pid, sizeof(pid), &value, sizeof(value), 0) == 0) {
                printf("page_swap_direct_reclaims_pid_%u: %llu\n", pid, (unsigned long long)value);
                total += value;
            }
        }
        printf("page_swap_direct_reclaims_total: %llu\n", (unsigned long long)total);
        sleep(1);
    }
