...
struct {
	...
	__type(key, __u32);
	__type(value, __u64);
} disk_read_stats SEC(".maps");

static __always_inline void update_stat(__u32 key, __u64 delta)
{
	__u64 *val = bpf_map_lookup_elem(&disk_read_stats, &key);
	if (val) {
		__sync_fetch_and_add(val, delta);
	} else {
		bpf_map_update_elem(&disk_read_stats, &key, &delta, BPF_ANY);
	}
}
...
int handle_vfs_read_ret(struct pt_regs *ctx)
{
	__u64 tid = bpf_get_current_pid_tgid();
	__u64 *start = bpf_map_lookup_elem(&ts_start_pid, &tid);
	if (!start)
		return 0;
	update_stat(0, bpf_ktime_get_ns() - *start);
	update_stat(1, 1);
...
int handle_blk_update_request(struct pt_regs *ctx)
{
	...
	if (!start)
		return 0;
	update_stat(2, bpf_ktime_get_ns() - *start);
	update_stat(3, 1);
